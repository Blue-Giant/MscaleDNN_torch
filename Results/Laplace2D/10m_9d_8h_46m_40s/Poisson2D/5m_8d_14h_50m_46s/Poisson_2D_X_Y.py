...
class PDE_DNN(tn.Module):
    ...
    def loss2in_X_Y(self, X=None, Y=None, fside=None, if_lambda2fside=True, loss_type='ritz_loss', relaxation2lncosh=0.1):
        """
        ...
        dUNN2x = torch.reshape(gradUNN2X[0], shape=[-1, 1])
        dUNN2y = torch.reshape(gradUNN2Y[0], shape=[-1, 1])

        if str.lower(loss_type) == 'ritz_loss' or str.lower(loss_type) == 'variational_loss':
            # the ritz loss: loss = 0.5*norm(U,2) - multiply(U,f)
            dUNN_2Norm = torch.square(dUNN2x) + torch.square(dUNN2y)
            ...
            grad_dUNN2XX = torch.autograd.grad(dUNN2x, X, grad_outputs=torch.ones_like(X), create_graph=True,
                                               retain_graph=True)
            grad_dUNN2YY = torch.autograd.grad(dUNN2y, Y, grad_outputs=torch.ones_like(Y), create_graph=True,
                                               retain_graph=True)

            ddUNN2xx = torch.reshape(grad_dUNN2XX[0], shape=[-1, 1])
            ddUNN2yy = torch.reshape(grad_dUNN2YY[0], shape=[-1, 1])
            # obtaining the point-wise error according to: -Laplace U=f --> -Laplace U - f --> -(Laplace U + f)
            loss_it_L2 = torch.add(ddUNN2xx, ddUNN2yy) + torch.reshape(force_side, shape=[-1, 1])
            ...
            ddUNN2yy = torch.reshape(grad_dUNN2YY[0], shape=[-1, 1])
            # obtaining the point-wise error according to: -Laplace U=f --> -Laplace U - f --> -(Laplace U + f)
            loss_it_temp = torch.add(ddUNN2xx, ddUNN2yy) + torch.reshape(force_side, shape=[-1, 1])
...
def Solve_PDE(Rdic=None):
    ...
    for i_epoch in range(Rdic['max_epoch'] + 1):
        ...
        UNN2train, loss_it = model.loss2in_X_Y(X=x_it_batch, Y=y_it_batch, fside=f, loss_type=Rdic['loss_type'],
        ...
        Uexact2train = u_true(x_it_batch, y_it_batch)
