...
class PDE_DNN(tn.Module):
    def __init__(self, input_dim=4, out_dim=1, hidden_layer=None, Model_name='DNN', name2actIn='relu',
        ...
        self.mat2X = torch.tensor([[1, 0, 0]], dtype=self.float_type,
                                  device=self.opt2device, requires_grad=False)  # 1 行 3 列
        self.mat2Y = torch.tensor([[0, 1, 0]], dtype=self.float_type,
                                  device=self.opt2device, requires_grad=False)  # 1 行 3 列
        self.mat2Z = torch.tensor([[0, 0, 1]], dtype=self.float_type,
                                  device=self.opt2device, requires_grad=False)  # 1 行 3 列

    def loss2in_X_Y_Z(self, X=None, Y=None, Z=None, fside=None, if_lambda2fside=True,
        ...
        gradUNN2X = torch.autograd.grad(UNN, X, grad_outputs=torch.ones_like(X), create_graph=True, retain_graph=True)
        gradUNN2Y = torch.autograd.grad(UNN, Y, grad_outputs=torch.ones_like(Y), create_graph=True, retain_graph=True)
        gradUNN2Z = torch.autograd.grad(UNN, Z, grad_outputs=torch.ones_like(Z), create_graph=True, retain_graph=True)

        dUNN2dX = torch.reshape(gradUNN2X[0], shape=[-1, 1])
        dUNN2dY = torch.reshape(gradUNN2Y[0], shape=[-1, 1])
        dUNN2dZ = torch.reshape(gradUNN2Z[0], shape=[-1, 1])

        if str.lower(loss_type) == 'ritz_loss' or str.lower(loss_type) == 'variational_loss':
            dUNN_2Norm = torch.square(dUNN2dX) + torch.square(dUNN2dY) + torch.square(dUNN2dZ)
            ...
            grad_dUNN2XX = torch.autograd.grad(dUNN2dX, X, grad_outputs=torch.ones_like(X), create_graph=True,
                                               retain_graph=True)
            grad_dUNN2YY = torch.autograd.grad(dUNN2dY, Y, grad_outputs=torch.ones_like(Y), create_graph=True,
                                               retain_graph=True)
            grad_dUNN2ZZ = torch.autograd.grad(dUNN2dZ, Z, grad_outputs=torch.ones_like(Z), create_graph=True,
                                               retain_graph=True)

            ddUNN2dXX = torch.reshape(grad_dUNN2XX[0], shape=[-1, 1])
            ddUNN2dYY = torch.reshape(grad_dUNN2YY[0], shape=[-1, 1])
            ddUNN2dZZ = torch.reshape(grad_dUNN2ZZ[0], shape=[-1, 1])

            # -Laplace U = f --> -Laplace U - f --> -(Laplace U + f)
            loss_it_L2 = ddUNN2dXX + ddUNN2dYY + ddUNN2dZZ + torch.reshape(force_side, shape=[-1, 1])
            ...
            # -Laplace U = f --> -Laplace U - f --> -(Laplace U + f)
            loss_it_temp = ddUNN2dXX + ddUNN2dYY + ddUNN2dZZ + torch.reshape(force_side, shape=[-1, 1])
...
def Solve_PDE(Rdic=None):
    ...
    f_side, u_true, u_left, u_right, u_behind, u_front, u_bottom, u_top = General_Laplace.get_infos2PDE_3D(
        input_dim=input_dim, out_dim=out_dim, intervalL=region_lb, intervalR=region_rt, equa_name=Rdic['equa_name'])
    ...
    for i_epoch in range(Rdic['max_epoch'] + 1):
        ...
        x_it_batch = dataUtilizer2torch.rand_in_1D(
            batch_size=batchsize_in, variable_dim=1, region_a=region_lb, region_b=region_rt,
            to_float=True, to_torch=True, to_cuda=Rdic['with_gpu'], gpu_no=Rdic['gpuNo'], use_grad=True)
        ...
        UNN2train, loss_it = model.loss2in_X_Y_Z(X=x_it_batch, Y=y_it_batch, Z=z_it_batch, fside=f_side,
        ...
        Uexact2train = u_true(x_it_batch, y_it_batch, z_it_batch)
