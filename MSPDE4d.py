...
class MscaleDNN(tn.Module):
    ...
    def loss_it2Laplace(self, XYZS=None, fside=None, if_lambda2fside=True, loss_type='ritz_loss'):
        ...
        shape2XYZS = XYZS.shape
        lenght2XYZS_shape = len(shape2XYZS)
        assert (lenght2XYZS_shape == 2)
        assert (shape2XYZS[-1] == 4)
        X = torch.reshape(XYZS[:, 0], shape=[-1, 1])
        Y = torch.reshape(XYZS[:, 1], shape=[-1, 1])
        Z = torch.reshape(XYZS[:, 2], shape=[-1, 1])
        S = torch.reshape(XYZS[:, 3], shape=[-1, 1])
        ...
        grad2UNN = torch.autograd.grad(UNN, XYZS, grad_outputs=torch.ones_like(X), create_graph=True, retain_graph=True)
        dUNN = grad2UNN[0]
        ...
        if str.lower(loss_type) == 'ritz_loss' or str.lower(loss_type) == 'variational_loss':
            ...
            dUNNxxyz = torch.autograd.grad(dUNN2x, XYZS, grad_outputs=torch.ones_like(X),
                                           create_graph=True, retain_graph=True)[0]
            dUNNyxyz = torch.autograd.grad(dUNN2y, XYZS, grad_outputs=torch.ones_like(X),
                                           create_graph=True, retain_graph=True)[0]
            dUNNzxyz = torch.autograd.grad(dUNN2z, XYZS, grad_outputs=torch.ones_like(X),
                                           create_graph=True, retain_graph=True)[0]
            dUNNSxyZ = torch.autograd.grad(dUNN2s, XYZS, grad_outputs=torch.ones_like(X),
                                           create_graph=True, retain_graph=True)[0]
            ...
            dUNNss = torch.reshape(dUNNSxyZ[:, 3], shape=[-1, 1])
    ...
    def loss2bd(self, XYZS_bd=None, Ubd_exact=None, if_lambda2Ubd=True):
        ...
        shape2XYZS = XYZS_bd.shape
        lenght2XYZS_shape = len(shape2XYZS)
        assert (lenght2XYZS_shape == 2)
        assert (shape2XYZS[-1] == 4)
        X_bd = torch.reshape(XYZS_bd[:, 0], shape=[-1, 1])
        Y_bd = torch.reshape(XYZS_bd[:, 1], shape=[-1, 1])
        Z_bd = torch.reshape(XYZS_bd[:, 2], shape=[-1, 1])
        S_bd = torch.reshape(XYZS_bd[:, 3], shape=[-1, 1])
