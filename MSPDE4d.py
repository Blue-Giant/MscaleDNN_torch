        ...
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            if R['loss_type'] == 'variational_loss':
                ...
                if R['PDE_type'] == 'general_Laplace':
                    ...
                    loss_it_variational = (1.0 / 2) *laplace_pow - tf.multiply(f(X_it, Y_it, Z_it, S_it), UNN)
                elif R['PDE_type'] == 'pLaplace':
                    a_eps = A_eps(X_it, Y_it, Z_it, S_it)                          # * 行 1 列
                    laplace_p_pow = a_eps*tf.pow(dUNN_2norm, p)
                    if R['equa_name'] == 'multi_scale4D_2':
                        fXYZS = MS_LaplaceEqs.get_force2pLaplace_4D(x=X_it, y=Y_it, z=Z_it, s=S_it, equa_name=R['equa_name'])
                        loss_it_variational = (1.0 / p) * laplace_p_pow - tf.multiply(tf.reshape(fXYZS, shape=[-1, 1]), UNN)
                    else:
                        loss_it_variational = (1.0 / p) * laplace_p_pow - tf.multiply(f(X_it, Y_it, Z_it, S_it), UNN)
