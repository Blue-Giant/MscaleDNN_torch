...
def solve_Multiscale_PDE(Rdic=None):
    ...
    logfile_name = '%s_%s.txt' % ('log2train', Rdic['name2act_hidden'])
    ...
    if Rdic['PDE_type'] == 'general_Laplace' or Rdic['PDE_type'] == 'Laplace':
        # -laplace u = f
        region_lb = 0.0
        region_rt = 1.0
        f, u_true, u_left, u_right, u_bottom, u_top = General_Laplace.get_infos2Laplace_2D(
            input_dim=input_dim, out_dim=out_dim, left_bottom=region_lb, right_top=region_rt, equa_name=Rdic['equa_name'])
    elif Rdic['PDE_type'] == 'pLaplace_implicit':
        mesh_number = Rdic['mesh_number']
        ...
        u_true, f, A_eps, u_left, u_right, u_bottom, u_top = MS_LaplaceEqs.get_infos2pLaplace_2D(
            input_dim=input_dim, out_dim=out_dim, mesh_number=Rdic['mesh_number'], pow_order2Aeps=Rdic['order2Aeps_MSE4'],
            intervalL=region_lb, intervalR=region_rt, equa_name=Rdic['equa_name'])
    elif Rdic['PDE_type'] == 'pLaplace_explicit':
        p_index = Rdic['order2pLaplace_operator']
    ...
    elif Rdic['PDE_type'] == 'Possion_Boltzmann':
        ...
        A_eps, kappa, u_true, u_left, u_right, u_top, u_bottom, f = MS_BoltzmannEqs.get_infos2Boltzmann_2D(
            equa_name=Rdic['equa_name'], intervalL=region_lb, intervalR=region_rt)

    model = MscaleDNN(input_dim=Rdic['input_dim'], out_dim=Rdic['output_dim'], hidden_layer=Rdic['hidden_layers'],
                      Model_name=Rdic['model2NN'], name2actIn=Rdic['name2act_in'], name2actHidden=Rdic['name2act_hidden'],
                      name2actOut=Rdic['name2act_out'], opt2regular_WB='L0', type2numeric='float32',
                      factor2freq=Rdic['freq'], sFourier=Rdic['sfourier'], repeat_highFreq=Rdic['repeat_High_freq'],
                      use_gpu=Rdic['use_gpu'], No2GPU=Rdic['gpuNo'])
    if True == Rdic['use_gpu']:
    ...
    if Rdic['testData_model'] == 'random_generate':
        ...
        # size2test = 100
        test_xy_bach = dataUtilizer2torch.rand_it(test_bach_size, Rdic['input_dim'], region_lb, region_rt, to_torch=False,
                                                  to_float=True, to_cuda=False, gpu_no=Rdic['gpuNo'])
        saveData.save_testData_or_solus2mat(test_xy_bach, dataName='testXY', outPath=R['FolderName'])
    else:
        if Rdic['PDE_type'] == 'pLaplace_implicit' or Rdic['PDE_type'] == 'pLaplace_explicit':
            test_xy_bach = Load_data2Mat.get_data2pLaplace(equation_name=Rdic['equa_name'], mesh_number=mesh_number)
            size2batch = np.shape(test_xy_bach)[0]
            size2test = int(np.sqrt(size2batch))
            saveData.save_meshData2mat(test_xy_bach, dataName='meshXY', mesh_number=mesh_number, outPath=Rdic['FolderName'])
        elif Rdic['PDE_type'] == 'Possion_Boltzmann':
            ...
            test_xy_bach = Load_data2Mat.get_randomData2mat(dim=Rdic['input_dim'], data_path='../dataMat_highDim')
            size2batch = np.shape(test_xy_bach)[0]
            size2test = int(np.sqrt(size2batch))
    ...
    if True == R['use_gpu']:
        test_xy_torch = test_xy_torch.cuda(device='cuda:' + str(Rdic['gpuNo']))

    file_path2model = '../pre_train'
    # save_load_NetModule.load_torch_net2file(path2file=file_path2model, model2net=mscalednn, name2model='MscaleDNN',
    #                                         opt_way2load='para')
    ...
    saveData.save_2testSolus2mat(utrue2test_numpy, unn2test_numpy, actName='utrue',
                                 actName1=Rdic['name2act_hidden'], outPath=Rdic['FolderName'])
