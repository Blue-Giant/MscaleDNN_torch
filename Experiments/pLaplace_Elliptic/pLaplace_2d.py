...
def solve_Multiscale_PDE(Rdic=None):
    ...
    for i_epoch in range(Rdic['max_epoch'] + 1):
        ...
        xy_it_batch = dataUtilizer2torch.rand_in_2D(
            batch_size=batchsize_it, variable_dim=Rdic['input_dim'],  region_left=region_lb, region_right=region_rt,
        ...
        loss_bd2right = model.loss2bd(XY_bd=xr_bd_batch, Ubd_exact=u_right, loss_type=Rdic['loss_type2bd'],
                                      relaxation2lncosh=Rdic['scale2lncosh'])
        loss_bd2bottom = model.loss2bd(XY_bd=yb_bd_batch, Ubd_exact=u_bottom, loss_type=Rdic['loss_type2bd'],
                                       relaxation2lncosh=Rdic['scale2lncosh'])
        loss_bd2top = model.loss2bd(XY_bd=yt_bd_batch, Ubd_exact=u_top, loss_type=Rdic['loss_type2bd'],
                                    relaxation2lncosh=Rdic['scale2lncosh'])
    ...
    plotData.plot_Hot_point_wise_err(point_square_error_numpy, size_vec2mat=size2test, actName=Rdic['name2act_hidden'],
                                     seedNo=Rdic['seed'], outPath=Rdic['FolderName'])

    save_load_NetModule.save_torch_net2file_with_keys(
        outPath=Rdic['FolderName'], model2net=model, name2model='MscaleDNN', optimizer=optimizer,
